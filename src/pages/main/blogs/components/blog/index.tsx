...
const [blogID, setBlogID] = useState<string>('')
  ...
  const {  isFetching } = useQuery(['singleBlog', blogID], () =>
              ...
              <div>
                  ...
                  <section className='flex justify-between items-center'>
                    <div>
                      ...
                      <p className='text-gray-500 text-sm mt-1'>
                       <span>Created on: {moment(blogData?.createdAt).format('MMM DD, YYYY') }</span>
                      {' | '} 
                        <small className='text-gray-400'>Last updated: {moment(blogData?.updatedAt).format('MMM DD, YYYY') }</small>
                      </p>
                    </div>
                    <div>
                      <Button
                ...
                <section className=''>
                    <article className='py-6 max-w-2xl'>
                      ...
                      <p className='text-gray-900 text-lg mt-3'>{blogData?.except}</p>
                      <div className='text-gray-700 mt-4'>{parse(blogData?.body)}</div>
                    </article>
                </section>
                </div>
              </>
